...
1. **Clean each dataset on its own** — fix column names, keep only NYC data, remove rows that don't apply (like non-bar businesses or subway exits with no entry), and pull out latitude/longitude where needed.
2. **Match each location to a neighborhood** — I checked which neighborhood box (using its min/max latitude and longitude) each bar, subway entrance, and complaint falls inside, then double-checked the match against the listed borough to catch mistakes.
3. **Group everything by neighborhood** — count bars, average nearby subway entrances, and average nearby complaints per neighborhood. I only kept neighborhoods with at least 5 bars so the numbers were meaningful.
4. **Score and rank** — scale each number to 0–100, then combine them into the final PopupIQ Score using the weights above.
5. **Save the results** — final rankings are saved to `popupiq_scores.csv`.
